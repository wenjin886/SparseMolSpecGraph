...
from GraphTMR_seq2mol import NMRSeq2MolGenerator
...
def main(args):
    ...
    nmr_tokenizer = PreTrainedTokenizerFast(tokenizer_file=args.nmr_tokenizer_path)
    formula_tokenizer = PreTrainedTokenizerFast(tokenizer_file=args.formula_tokenizer_path)
    ...
    if args.code_test:
        train_set = NMR2MolDataset(src_data=osp.join(args.data_dir_path, "src-val.txt"), 
                                tgt_data=osp.join(args.data_dir_path, "tgt-val.txt"), 
                                formula_tokenizer=formula_tokenizer, nmr_tokenizer=nmr_tokenizer, tgt_tokenizer=tgt_tokenizer)
        train_dataloader = create_nmr2mol_dataloader(train_set, batch_size=args.batch_size,  num_workers=17, shuffle=False)
        ...
        val_dataloader = create_nmr2mol_dataloader(val_set, batch_size=args.batch_size, num_workers=17, shuffle=False)
        test_dataloader = create_nmr2mol_dataloader(test_set, batch_size=args.batch_size, num_workers=17, shuffle=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nmr_tokenizer_path', type=str, required=True)
