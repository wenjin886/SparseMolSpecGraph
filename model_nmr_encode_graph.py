...
class NodeFeatureEmbedding(nn.Module):
    def __init__(self, mult_class_num, nH_class_num, 
        ...
        self.mult_embed = nn.Embedding(mult_class_num, feature_dim)
        self.nH_embed = nn.Embedding(nH_class_num, feature_dim)
        ...
        self.feature_w = nn.Linear(feature_dim, 1)
        self.dropout = nn.Dropout(dropout)
        self.proj = nn.Sequential(nn.Linear(feature_dim, feature_dim*2), 
                                  nn.SiLU(),
    ...
    def forward(self, centroid, width, nH, multiplet):
        ...
        x = (x * feature_w).sum(dim=1)   # [N, feature_dim]

        return self.proj(self.dropout(x)) # [N, feature_dim]
...
class NMRGraphEncoder(nn.Module):
    def __init__(self, in_node_dim, hidden_node_dim, 
        ...
        if edge_dim is None:
            ...
            self.use_edge_embed = False
            edge_dim = 1
        else:
            self.use_edge_embed = True
